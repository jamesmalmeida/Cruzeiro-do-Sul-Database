...
from .models import Experiment, Beamline, Facility, User, Element, Normalization
...
class AddNormalization(CreateView):
    ...
    success_url = reverse_lazy('add-normalization')
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if file.name.endswith('.txt') or file.name.endswith('.csv'):
                    # Carregue o arquivo em um DataFrame pandas
                    df = pd.read_csv(file)
